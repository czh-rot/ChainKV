...
type DatabaseWriter interface {
	...
	// 得看是谁调用，该Put可以是db.Put，也可以是batch.Put
...
func (t *Trie) TryUpdate(key, value []byte) error {
	k := keybytesToHex(key) //转为hex编码
	//fmt.Println(k)
	if len(value) != 0 { //value不为空
		_, n, err := t.insert(t.root, nil, k, valueNode(value)) //插入
		if err != nil {
			return err
		}
		t.root = n
...
func (t *Trie) insert(n node, prefix, key []byte, value node) (bool, node, error) {
	...
	if len(key) == 0 {
		...
		if v, ok := n.(valueNode); ok {
			//判断旧的value和新的value是否相等，递归调用结束返回上层并赋值
	...
	switch n := n.(type) {
	case *shortNode:
		// 取要插入的key和当前遍历node的Key的共同前缀长度
		matchlen := prefixLen(key, n.Key)
		// 如果当前插入的key和当前遍历节点的Key完全匹配，说明两个Key是一样的，如果value也一样，
		// 则dirty为false，返回错误
		if matchlen == len(n.Key) {
			...
			// 如果没有错误则将新插入的Value赋值到shortNode的Val并返回，这里是叶子节点
			// t.newFlag()会为Flag字段中的dirty和gen赋值
		...
		var err error
		//fmt.Println(n.Key[matchlen],prefix,n.Key[:matchlen+1],n.Key[matchlen+1:])
		_, branch.Children[n.Key[matchlen]], err = t.insert(nil, append(prefix, n.Key[:matchlen+1]...), n.Key[matchlen+1:], n.Val)
		if err != nil {
			return false, nil, err
		}
		// 和row268一样，递归调用共生成两个shortNode
		...
		// 更新原来的shortNode，并令其Val为fullNode
...
func (t *Trie) resolveHash(n hashNode, prefix []byte) (node, error) {
	...
	// 根据hash去数据库中取value
	enc, err := t.db.Get(n) //根据hash取出value
	if err != nil || enc == nil {
...
func (t *Trie) Hash() common.Hash {
	...
	t.root = cached
	return common.BytesToHash(hash.(hashNode)) //字节数组变成32字节长
}
