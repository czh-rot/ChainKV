...
//一个接口的实现，把文件描述符作为参数，在Storage.Create中调用，返回值为&iStorageWriter{w, c}，c为iStorage，w为Storage.Writer
func (c *iStorage) Create(fd storage.FileDesc) (storage.Writer, error) {
	w, err := c.Storage.Create(fd)
	return &iStorageWriter{w, c}, err
}
func (c *iStorage) Create_s(fd storage.FileDesc) (storage.Writer, error) {
	w, err := c.Storage.Create_s(fd)
	return &iStorageWriter{w, c}, err
}
