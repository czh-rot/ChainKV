...
var (
	...
	DefaultCompactionL0Trigger           = 4 //L0层最大的sst数量？
...
const (
	DefaultCompression Compression = iota // 0
	NoCompression // 1
	SnappyCompression // 2
	nCompression // 3
)
...
func (o *Options) GetCompactionTotalSize(level int) int64 {
	...
	if mult == 0 {
		mult = math.Pow(DefaultCompactionTotalSizeMultiplier, float64(level)) //10的平方
	}
	return int64(float64(base) * mult)  //base=10m
}
