...
//当日志阅读器删除一个block或chunk时，将调用Drop方法，也就是落盘时会调用drop
...
////读日志记录，以32KB的BLOCK为单位，一直到Last类型，表明日志记录读取完毕，获取日志Reader
//获取singlReader，读取一个chunk并校验，是否是最后一个chunk
...
type Writer struct {
	...
	w io.Writer
	// seq is the sequence number of the current journal.是根据序列号对当前日志进行操作
	seq int
	// f is w as a flusher.
	f flusher
...
func NewWriter_s(w io.Writer) *Writer2 {
	f, _ := w.(flusher)
	return &Writer2{
		w: w,
		f: f,
...
func (w *Writer2) fillHeader(last bool) {
	if w.i+headerSize > w.j || w.j > blockSize {
		panic("leveldb/journal: bad writer state")
	}
	if last {
...
func (w *Writer2) Reset(writer io.Writer) (err error) {
	w.seq++
	if w.err == nil {
		w.writePending()
		err = w.err
...
func (w *Writer2) Next() (io.Writer, error) {
	w.seq++
	if w.err != nil {
		return nil, w.err
	}
	...
	w.first = true
	w.pending = true
	return singleWriter2{w, w.seq}, nil
}
//写一条日志记录
...
//写日志，开始写chunk，过程中不断判断buffer是否超过32KB
...
func (x singleWriter2) Write(p []byte) (int, error) {
	w := x.w
	if w.seq != x.seq {
		return 0, errors.New("leveldb/journal: stale writer")
	}
