...
var benchDB = filepath.Join("E:/DB/1")
type dbBench struct {
	b    *testing.B
...
func openDBBench2(b *testing.B, noCompress bool) *dbBench {
	_, err := os.Stat(benchDB)
	if err == nil {
		err = os.RemoveAll(benchDB)
		if err != nil {
...
func (p *dbBench) populate(n int) {
	p.keys, p.values = make([][]byte, n), make([][]byte, n)//keys values包含了所有的kv对
	v := newValueGen(0.5)
	for i := range p.keys {
		p.keys[i], p.values[i] = []byte(fmt.Sprintf("%016d", i)), v.get(100)
	}
}
func (p *dbBench) populate2(n int) {
	p.keys, p.values = make([][]byte, n), make([][]byte, n)//keys values包含了所有的kv对
	v := newValueGen(0.5)
	for i := range p.keys {
		token:=make([]byte,16)
...
func (p *dbBench) writes(perBatch int) {
	...
	j := 0
	for i := range batches {//把数据写入batch
		first := true
...
func (p *dbBench) writes2(perBatch int) {
	b := p.b
	db := p.db
...
func (p *dbBench) Mergewrites(perBatch int) {
	b := p.b
	db := p.db
...
func (p *dbBench) puts2() {
	...
	b.StartTimer()
	for i := range p.keys {
		err := db.Put_s(p.keys[i], p.values[i], p.wo)
		if err != nil {
			b.Fatal("put failed: ", err)
		}
	}
...
func BenchmarkDBWrite_sBatch(b *testing.B) { //测试新列写
	p := openDBBench(b, false)
	p.populate2(b.N)
	p.writes2(1000)
	p.close()
...
func BenchmarkDBWriteBatchMerge(b *testing.B) { //混合写
	p := openDBBench(b, false)
	p.populate2(b.N)
	p.Mergewrites(1000)
	p.close()
...
func BenchmarkDBWriteBatchMergeReaL(b *testing.B) {
	p := openDBBench2(b, false) //内存设为2+2
	p.populate2(b.N)
	p.Mergewrites(1000)
	p.close()
}

func BenchmarkDBWriteBatchMergeWithCompression(b *testing.B) {
...
func BenchmarkDBPut_s(b *testing.B) {
	p := openDBBench(b, false)
	p.populate(b.N)
	p.puts2()
	p.close()
}
func BenchmarkDBMergePut(b *testing.B) {
	p := openDBBench(b, false)
	p.populate(b.N)
	p.Mergeputs()
	p.close()
